...
The response schema is generated from `Outputs`. `Vision.GameCapture` makes one model request per reader. The model must return a `readable` decision and a typed `value` for numeric outputs. A value is published only when it is readable, present, correctly typed, and passes its optional configured validators. Unreadable, contradictory, type-invalid, or out-of-range results are reported in `Failures` and omitted from `Values`.
